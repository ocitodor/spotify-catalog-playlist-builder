...
# ---------------------------------------------------------------------------
CLIENT_ID = os.environ.get("SPOTIPY_CLIENT_ID", "PASTE_CLIENT_ID_HERE")
CLIENT_SECRET = os.environ.get("SPOTIPY_CLIENT_SECRET", "PASTE_CLIENT_SECRET_HERE")
...
def fetch_tracks(sp: spotipy.Spotify, albums: list, artist_id: str) -> list:
    ...
    Economizes API calls: albums with 10 or fewer tracks need one call,
    and a short pause between calls keeps request bursts polite."""
    import time

    seen, tracks = set(), []
    for n, album in enumerate(albums, 1):
        if n % 10 == 0:
            print(f"  fetching tracks... {n}/{len(albums)} releases")
        offset = 0
        while True:
            ...
            if page["next"] is None:
                break
            # Stop early if we already have everything the album contains —
            # saves a wasted "next page is empty" API call.
            if album.get("total_tracks") and offset + 10 >= album["total_tracks"]:
                break
            offset += 10
            time.sleep(0.2)
    return tracks
